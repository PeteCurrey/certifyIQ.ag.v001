...
export default function CalendarPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true)
        const [slotsRes, bookingsRes, assessorsRes] = await Promise.all([
          supabase
            .from('scheduled_slots')
            .select('*, bookings(booking_ref, service_type, properties(address_line_1, town))')
            .neq('status', 'cancelled'),
          supabase
            .from('bookings')
            .select('id, booking_ref, service_type, properties(address_line_1, town, postcode)')
            .eq('status', 'paid')
            .is('confirmed_datetime', null),
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'scheduled_slots' }, load)
      .subscribe()
  ...
  const eventStyleGetter = (event: CalEvent) => {
    const color = SERVICE_COLORS[event.serviceType] || '#6b7280'
    return {
      style: {
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.sidebar}>
          ...
          <div className={styles.unscheduledList}>
            {unscheduled.length === 0 && (
              <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>All jobs scheduled ✓</p>
            )}
            {unscheduled.map(b => (
              <div
                key={b.id}
              ...
              >
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>
                  {b.properties?.address_line_1}, {b.properties?.town}
                </div>
                <div style={{ color: 'var(--accent-lime)', fontSize: '0.7rem', marginTop: '0.25rem' }}>
                  Click to schedule →
                </div>
              </div>
            ))}
        ...
        <div className={styles.calendarContainer}>
          <BigCalendar
            ...
            onSelectEvent={(evt: CalEvent) => {
              setActiveBookingId(evt.bookingId)
              setDrawerOpen(true)
            }}
            onSelectSlot={(slotInfo: any) => {
              // Clicking on an empty slot — in a real app this would open a quick-schedule dialog
